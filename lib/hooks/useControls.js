...
function useControls(props) {
    ...
    const controlShapeReactive = (0, vue_1.reactive)({
        x: 0,
        y: 0,
        width: 0,
        height: 0,
    });
    const imageScaleRef = (0, vue_1.computed)(() => {
        const { width = 0, height = 0 } = imageRef.value || {};
        return (0, scaleCompute_1.default)({ width, height }, sizeRef.value);
    });
    const cropShapeRef = (0, vue_1.computed)(() => {
    ...
    const onContainerDrag = (0, useDrag_1.default)(({ x, y }) => {
    ...
    const controls = [
        {
            directions: ['n', 'w'],
            onDrag: (0, useDrag_1.default)(({ x, y }) => {
